...
const getBookingStats = async () => {
    ...
    const bookingsLastOneYearPromise = Booking.countDocuments({
        createdAt: { $gte: oneYearAgo }
    })

    const totalBookingByUniqueUsersPromise = Booking.distinct("user").then((user: any) => user.length)

    const [totalBooking, totalBookingByStatus, bookingsPerTour, avgGuestCountPerBooking, bookingsToDay, bookingsLast7Days, bookingsLast30Days, bookingsLast90Days, bookingsLastOneYear, totalBookingByUniqueUsers] = await Promise.all([
        totalBookingPromise,
